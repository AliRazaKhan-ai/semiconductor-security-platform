...
def configure_cli_logging() -> None:
    """Suppress noisy third-party DEBUG output in terminal commands."""
    for logger_name in (
        "web3",
        "urllib3",
        "rlp",
...
def build_stopped_pipeline_result(
    simulation: dict[str, Any],
    gate: Any,
) -> dict[str, Any]:
    """Build a fail-closed result without invoking compliance or blockchain."""
    return {
        "decision": {
            "decision": "DENIED",
            "status": "FAILED",
            "deployment_recommendation": gate.deployment_decision,
        ...
        "pipeline": {
            ...
            "classification": gate.classification,
            "dashboard_status": gate.dashboard_status,
            "alert_color": gate.alert_color,
        },
        "blockchain": None,
        "blockchain_error": None,
        "report": None,
        "stopped_before_compliance": True,
    }
...
def run_scan(
    ...
    *,
    ...
    with application.test_client() as client:
        ...
        result["security_gate"] = gate.to_dict()

        if not gate.passed:
            result["compliance"] = build_stopped_pipeline_result(
                simulation,
                gate,
            )
            return result
...
def run_server(args: argparse.Namespace) -> int:
    ...
    socketio.run(
        ...
        debug=debug,
        use_reloader=debug,
        # This launcher is restricted to local development/demo use.
        # A production deployment should use the configured WSGI server.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    handlers = {
        "scan": command_scan,
        "pipeline-run": command_pipeline_run,
